...
struct Shooting_Method
{
		...
		int solve(double& x4, double& y4) {
			...
			begin:
				...
				else
				{
					...
					double limit1 = 150000, limit2 = 200;
	...
	Shooting_Method()
	{
		...
		//equation = &Shooting_Method::equations_for_2d_with_pulsations; 	n = 10;  bc = { 0, 1, 5, 7, 8 };
		//equation = &Shooting_Method::equations_for_2d_with_pulsations; 	n = 8;  bc = { 0, 1, 5, 6 };

		equation = &Shooting_Method::equations_for_2d_test; 	n = 8;  bc = { 0, 1, 5, 6 };
	...
	void equations_for_2d_with_pulsations(cd ld, double Ra, int i, int n, cd** y, int j, double x, cd& f)
	{
		...
		case 4: // C
			...
			break;
		case 5: // C''
			f = Le * (-I * y[j][1] * cosG / k - y[j][0] * sinG) + pow(k, 2) * y[j][4] + Le * ld * y[j][4];
			break;
		case 6: // T
			f = y[j][7]; // T'
			break;
	...
	void equations_for_2d_test(cd ld, double Ra, int i, int n, cd** y, int j, double x, cd& f)
	{
		#pragma warning(push)
		#pragma warning(disable: 4005)
		...
		case 2:		  	 // w''
			...
			break;
		case 3:			// w''''
			f = (ld) * (wzprime2 - pow(k, 2) * wz)  // ∇^2 * λ / Pr
				+ 2 * pow(k, 2) * wzprime2 - pow(k, 4) * wz   // rest of ∇^4
				+ pow(k, 2) * Ra / Pr * sinG * FF
				+ I * k * Ra / Pr * cosG * FFprime
				+ A1 * Wprime2
				- A2 * I * k * y[j][9]
				- A3 * I * k * y[j][9]
				- A4 * pow(k, 2) * y[j][8]
		...
		case 4: // C
			...
			break;
		case 5: // C''
			f = Pr * Le * (-I * y[j][1] * cosG / k - y[j][0] * sinG) + pow(k, 2) * y[j][4] + Pr * Le * ld * y[j][4];
			break;
		case 6: // T
			f = y[j][7]; // T'
			break;
		case 7: //T''
			f = Pr * (-I * y[j][1] * cosG / k - y[j][0] * sinG) + pow(k, 2) * y[j][6] + ld * y[j][6] * Pr;
			break;
		case 8:
			f = y[j][9];
			break;
